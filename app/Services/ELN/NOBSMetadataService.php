...
/**
 * NOBS ELN metadata extraction service.
 *
 * Handles extraction of sample names, folder paths, molecules,
 * and other metadata specific to NOBS ELN exports.
 */
class NOBSMetadataService implements ELNMetadataExtractorInterface
{
    ...
    /**
     * Validate the NOBS metadata structure.
     */
    public function validateMetadata(array $metadata): bool
    {
        // Basic validation for NOBS JSON-LD structure
        $requiredFields = ['@context', '@type', 'name', 'hasPart'];

        foreach ($requiredFields as $field) {
            if (! isset($metadata[$field])) {
                Log::warning("Missing required field in NOBS metadata: {$field}");
        ...
        if ($metadata['@type'] !== 'Study') {
            Log::warning("Invalid @type in NOBS metadata. Expected 'Study', got: ".($metadata['@type'] ?? 'null'));
        ...
        if ($metadata['@context'] !== 'https://schema.org') {
            Log::warning("Invalid @context in NOBS metadata. Expected 'https://schema.org'");
