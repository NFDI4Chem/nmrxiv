<template>
  ...
  >
    <div class="pt-5 px-5 bg-gray-200">
      <span v-if="study.study_preview_urls && study.study_preview_urls.length > 0">
        ...
        <span v-else>
          <div class="relative">
            <span v-for="(url, index) in study.study_preview_urls" :key="url">
              <span v-if="index == this.selectedPreviewIndex">
                <img class="h-36 w-full rounded-t-md shadow-lg" :src="url" alt="" />
              </span>
            </span>
            ...
            >
              <div>
                <ol role="list" class="flex items-center space-x-2">
                  <li v-for="(url, index) in study.study_preview_urls" :key="index">
                    <a
                      @click.prevent="selectedPreviewIndex = index"
                      v-if="index === selectedPreviewIndex"
          ...
          <span class="bg-gradient-to-r from-cyan-500 to-blue-500"> </span>
        </span>
      </span>
      <span v-else>
        <img
