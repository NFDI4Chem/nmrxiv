<template>
  <jet-dialog-modal :show="showDialog" :max-width="'6xl'" @close="showDialog = false">
    <template #title> {{ project.name }} - Manage Authors 
        <button
              type="button"
            ...
            <button
              type="button"
              v-else
              @click="displayAddAuthorForms = false"
    ...
    <template #content>
      <div>
        <div v-if="displayAddAuthorForms">
          ...
          >
            <div v-if="!isEdit" class="pb-36 px-4 sm:px-6 lg:pb-5 lg:px-0 lg:row-start-1 lg:col-start-1">
              <!-- Import Section -->
              <div>
                ...
                <div>
                  <div style="max-height: 40vh" class="overflow-auto p-2 mt-4">
                    <div
                    ...
                    >
                      ...
                      >
                        <label for="items" class="font-medium text-teal-900"
                          >{{ author.firstName }}
                          {{ author.lastName }}
                        </label>
                        <p
                          v-if="author.authorAffiliationDetailsList"
                  ...
                  <div v-if="fetchedAuthors.length > 0" class="sm:col-span-6 mt-4">
                    <jet-secondary-button
                      :disabled="!(this.selectedFetchedAuthorsList(false).length > 0)"
                      class="float-right ml-2"
                      @click="save('addSelected')"
                    >
                      Add Selected ({{ selectedFetchedAuthorsList(false).length }})
                    </jet-secondary-button>
                    <jet-secondary-button class="float-right" @click="save('addAll')">
                      Add All
                    </jet-secondary-button>
                  </div>
                </div>
            ...
            <div class="pb-36 px-4 sm:px-6 lg:pb-5 lg:px-0 lg:row-start-1 lg:col-start-2">
              <div class="p-2">
                <!--Add Manual Section-->
                <p class="text-sm leading-6 font-bold text-gray-900"><span v-if="!isEdit">Add</span><span v-else></span> Author</p>
                <div class="mt-1 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                  <div class="sm:col-span-2">
                  ...
                  <div v-if="!isEdit" class="sm:col-span-6 float-left">
                    <jet-secondary-button
                      class="float-right"
                      :disabled="
                    ...
                    <jet-secondary-button
                      ...
                      :disabled="
                        !(this.form && this.form.family_name && this.form.given_name)
                      "
                      @click="this.form.reset(), this.authorsForm.reset()"
                    >
                      Clear
                    </jet-secondary-button>
                  </div>
                  <div v-else class="sm:col-span-6 float-left">
                    <jet-secondary-button
                      class="float-right"
                      :disabled="
                        !(this.form && this.form.family_name && this.form.given_name)
                    ...
                    <jet-secondary-button
                      ...
                      :disabled="
                        !(this.form && this.form.family_name && this.form.given_name)
                      "
                      @click="this.displayAddAuthorForms = false, this.isEdit=false"
                    >
                      Cancel
                    </jet-secondary-button>
                  </div>
        ...
        <div v-if="authors.length > 0 && !displayAddAuthorForms" class="sm:rounded-md overflow-y-scroll">
          <draggable
              ...
              <div class="overflow-auto">
                <ul role="list" class="divide-y divide-gray-900">
                  <li>
                    ...
                    >
                      <div class="flex items-center cursor justify-between">
                        ...
                        <button
                          ...
                          @click="
                            (showManageRoleDialog = true),
                              (this.updateRoleForm.author_id = element.id)
                          "
                        >
                          <p
                            v-if="element.pivot && element.pivot.contributor_type"
                            class="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800"
                          >
                            {{
                              element.pivot.contributor_type
                                ? element.pivot.contributor_type
                                : "Researcher"
                            }}
                          </p>
      ...
      <div class="py-5" v-if="authors.length == 0 && !displayAddAuthorForms">
        <div class="text-center">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900">No Authors Listed</h3>
      ...
      <jet-dialog-modal :show="confirmDelete" @close="confirmDelete = false">
        ...
        <template #footer>
          <jet-secondary-button
            @click="confirmDelete = false && this.authorsForm.reset()"
          >
            Cancel
          </jet-secondary-button>
  ...
  <jet-dialog-modal :show="showManageRoleDialog" @close="showManageRoleDialog = false">
        ...
        >
          ...
          >
            <div class="text-gray-900">
              <b>{{ item.title }}</b> <br />
              <p class="text-xs align-top" v-html="item.description"></p>
            </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    fetchAuthors() {
      ...
      axios
        ...
        .catch(() => {
          this.error = "Something went wrong. Please check the input and try again.";
        })
        .finally(() => {
        ...
        authorsList.push({
          ...
          orcid_id:
            author.authorId && author.authorId.type == "ORCID"
              ? author.authorId.value
              : "",
          affiliation: author.authorAffiliationDetailsList.authorAffiliation[0]
            .affiliation
    ...
    addImportedAuthor(addAll) {
      ...
      if (this.authors.length > 0) {
        ...
        this.authors = this.formatAuthors(this.selectedFetchedAuthorsList(addAll));
      }
      this.executeQuery();
    },
    ...
    updateRole(role) {
      ...
      this.updateRoleForm.post(route("author.updateRole", this.project.id), {
        preserveScroll: true,
        onSuccess: () => {
          Inertia.reload({ only: ["project"] });
          this.loadInitial();
