                                            ...
                                            >
                                                ...
                                                >
                                                    <button
                                                        type="button"
                                                        :disabled="
                                                            !sampleBagitArchiveReady
                                                        "
                                                        :title="
                                                            sampleBagitArchiveReady
                                                                ? null
                                                                : sampleBagitStatusLabel +
...
export default {
    ...
    computed: {
        ...
        currentStudyData() {
            return this.currentStudy?.data ?? this.currentStudy ?? null;
        },
        sampleBagitJobStatus() {
            const study = this.currentStudyData;

            return (
                study?.metadata_bagit_generation_status ||
                (study?.bagit_archive_link ? "completed" : "pending")
            );
        },
        sampleBagitArchiveReady() {
            return (
                this.sampleBagitJobStatus === "completed" &&
                Boolean(this.currentStudyData?.bagit_archive_link)
            );
        },
        sampleBagitStatusLabel() {
            switch (this.sampleBagitJobStatus) {
                case "pending":
                    return "Queued";
                case "processing":
                    return "Processing";
        ...
        showSampleBagitDownloadOption() {
            ...
            return Boolean(
                this.currentStudyData && path.startsWith("/sample/")
            );
        },
        workspace() {
    ...
    methods: {
        ...
        requestSampleBagitDownload() {
            if (!this.sampleBagitArchiveReady) {
                return;
            }

            this.requestDownload(
                this.currentStudyData.bagit_archive_link,
                this.trackingIdentifier(this.currentStudyData)
            );
        },
